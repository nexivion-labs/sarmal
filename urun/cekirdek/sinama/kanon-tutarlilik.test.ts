...
const KANON_SHA = [
  ...
  "ork.sar:f04dc8e4ca60d7734fc0573cb8fe58963e8dc4fe84576305ef9bdaeecb85d9f9",
...
test("resmi sekizli 160/160 maddeyi, örneği ve dört parçayı sabit SHA'larla taşır", () => {
  const olcum = kanonMaddeleriniOlc(KOK);
  assert.equal(olcum.maddeler.length, 160);
  assert.equal(new Set(olcum.maddeler.map((m) => m.kod)).size, 160);
  assert.deepEqual(
...
test("A03 türevi idempotenttir ve geneldurum'u hüküm kaynağı olarak okumaz", () => {
  ...
  assert.match(ilk, /Resmi sonuç 160\/160 maddedir/);
