...
test("② + ④ temiz kopyada on altı yüzün ilk ve ikinci koşusu sıfır fark verir", () => {
  ...
  assert.deepEqual({ madde: ilk.kanonMaddesi, arac: ilk.aracSayisi }, { madde: 160, arac: 18 });
...
test("② kırmızı fikstür: üretilen bölgeye elle dokunan bayt ilk koşuda yakalanır ve onarılır", () => {
  ...
  writeFileSync(readme, readFileSync(readme, "utf8").replace("160 tekil madde", "149 tekil madde"), "utf8");
  ...
  assert.ok(readFileSync(readme, "utf8").includes("160 tekil madde"), "nöbet yüzü kanonik ölçüme onarmadı");
