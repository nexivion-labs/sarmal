...
export function kanonMaddeleriniOlc(kokYolu: string): { maddeler: readonly KanonMaddesiOlcumu[]; muhurler: readonly string[]; demet: string } {
  ...
  if (dosyalar.length !== 8 || maddeler.length !== 159 || kodlar.size !== 159) {
    throw new Error(`Resmi kanon 159/159 değil: ${dosyalar.length} dosya, ${maddeler.length}/${kodlar.size} madde.`);
  }
...
export function kanonTutarlilikMetni(kokYolu: string): string {
  ...
  const tutarlilik: Array<[string, string, string | number, boolean]> = [
    ...
    ["Kanon maddesi", "159 tekil", new Set(olcum.maddeler.map((m) => m.kod)).size, new Set(olcum.maddeler.map((m) => m.kod)).size === 159],
    ["Rol dağılımı", `${beklenenKarar} Karar + ${beklenenKural} Kural`, `${karar} Karar + ${kural} Kural`, karar === beklenenKarar && kural === beklenenKural],
    ["Dört parçalı madde", "159/159", `${159 - eksikDortParca.length}/159`, eksikDortParca.length === 0],
    ["Kanonik örnek", "159/159", `${159 - eksikOrnek.length}/159`, eksikOrnek.length === 0],
