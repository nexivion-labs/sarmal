...
Kanon 151 maddelik sekiz bölümden oluşur ve yalnız `yasa/kanon/` altında yaşar. Katı üretim omurgası `ÇalışmaAlanı → Auth → Proje[*] → Faz → Blok → Katman → AltKatman → Adım → üretir → Meyve → dosya` zinciridir. Esnek rejim bilinçli bir istisnadır; gerekçesi açık yazılır.
