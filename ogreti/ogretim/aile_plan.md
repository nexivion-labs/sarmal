...
Canlı örnek bu kartın kendi doğum yeridir. `is/plan/blok/kaynak_disiplini.sar` dosyasında `Katman( kod: KYN-OGRETIM, kullanır: TAKIM-CEKIRDEK, ad: "öğretim yüzü"` ilanı yer alır. O dalın altında `AltKatman( kod: KYN-OGRETIM-K1, departman: kodlama, ad: "öğretim üreteci"` bulunur ve departman eksenini bildirir. `KYN-OGR-A01` yaprağı öğretim üretecini yazmış, `Adım( kod: KYN-OGR-A02, durum: geliştirmede, öncelik: p1` yaprağı ise okumakta olduğunuz anlatıyı üretmiştir. Zaman ekseni de aynı ağaçta canlıdır: aynı dosyadaki gövde `Blok( kod: BLK-KYN, ad: "kaynak disiplini", mevsim: FAZ-2026-EYLUL,` satırıyla yürürlükteki mevsime kendi dosyasından `mevsim` kenarıyla bağlanır, o mevsim de `is/plan/faz/faz.sar` dosyasında `Faz( kod: FAZ-2026-EYLUL, ad: "Lansman Mevsimi"` olarak ilan edilir. Bu düğümlerin hangisini açmak istersen kodunu `gezin` aracına sor; araç tanımın yerini ve ona atıf veren her satırı birlikte söyler.
<!-- /SARMAL:ANLATI -->
